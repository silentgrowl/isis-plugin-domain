...
module Isis
  module Plugin
    class Domain < Isis::Plugin::Base
      ...
      def response_md
        @commands[1] = SLACK_REGEX.match(@commands[1])[:url] # Slack auto-linkifies domains, rewrite cleaned domain
        result = domain_search
        return %Q(Sorry, I can't process that name) unless result
        if result[:r].available?
          %Q(#{result[:domain]} is *available*! <#{registration_url(result[:domain])}|Register it!>)
        else
